...
// PRODUCTION: Always use /api for production deployment
// The .htaccess file will proxy /api/* to http://localhost:3000/api/*
const API_BASE = import.meta.env.VITE_API_BASE_URL || '/api';

// Force fallback mode only in unsupported environments (not in Replit development or production)
const USE_FALLBACK = window.location.hostname.includes('webcontainer') || window.location.hostname.includes('stackblitz');

console.log('API_BASE configured as:', API_BASE, 'Fallback mode:', USE_FALLBACK, 'Environment:', import.meta.env.MODE);
