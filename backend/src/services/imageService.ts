...
export class ImageService {
  async uploadImages(files: Express.Multer.File[], propertyId: string, isMain: boolean = false) {
    ...
    const maxSort = await prisma.propertyImage.findFirst({ where: { property_id: propertyId }, orderBy: { sort_order: 'desc' }, select: { sort_order: true } });
    let sortOrder = (maxSort?.sort_order || 0) + 1;

    if (isMain) {
      await prisma.propertyImage.updateMany({ where: { property_id: propertyId }, data: { is_main: false } });
    }
    ...
    for (const file of files) {
      ...
      const image: any = await prisma.propertyImage.create({
  ...
  async setMainImage(propertyId: string, imageId: string) {
    ...
    await prisma.propertyImage.updateMany({ where: { property_id: propertyId }, data: { is_main: false } });
    await prisma.propertyImage.update({ where: { id: imageId }, data: { is_main: true } });
